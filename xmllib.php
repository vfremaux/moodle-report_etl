...
if (!function_exists('recordstoxml')) {
    ...
    */
            ...
            foreach ($array as $key => $element) {
                if (is_object($element)) {
                    ...
                    foreach ($fields as $fieldname => $fieldvalue) {
                        ...
                        $processedfield = str_replace("& ", "&amp;amp; ", $fieldvalue);
                        if (preg_match("/<|>/", $fieldvalue)) {
                            $xml .= "\t\t<{$fieldname}><![CDATA[ ".$processedfield." ]]></{$fieldname}>\n";
                        } else {
                            $xml .= "\t\t<{$fieldname}>".$processedfield."</{$fieldname}>\n";
                        }
                    }
                    $xml .= "\t</{$baseelement}>\n";
                }
    ...
    /**
     * same but convert one singlerecord
     */
    function recordtoxml($record, $recordid, $recordelement, $subrecords = '') {
        global $strings;

        $baseelement = strtolower($recordelement); // calibrates the base name

        $xml = '';
        if (is_object($record)) {
            $fields = get_object_vars($record);
            ksort($fields);
            $xml = "\t<{$baseelement} id=\"$recordid\">\n";
            $xml .= "\t\t<ix>$recordid</ix>\n";
            foreach ($fields as $fieldname => $fieldvalue) {
                $processedfield = str_replace("& ", "&amp;amp; ", $fieldvalue);
                if (preg_match("/<|>/", $fieldvalue)) {
